...
The bioinformatics and RAG tools (GeoDownload, DiffExpression, WGCNA, Enrichment,
PPINetwork, RAG indexing/query, etc.) are listed below under "Available tools" with
their live signatures - use those exactly as given.
...
- For any other cross-step data passing (e.g. feeding a file path or gene list from
  one tool into the next), reference the exact output key from the previous step's
  result - either as `"{{ key_name }}"` or as the bare key name itself (e.g.
  `"expression_file":"deg_file"`) - and it will be resolved automatically from that
  step's output before the tool runs. Do not invent values that weren't actually
  returned by a previous step.
- CODE steps can also read `work/last_tool_output.json` to access the most recent
  tool's full raw output.
